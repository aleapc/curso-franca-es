...
>
  ...
  {#if sua}
    <p class="mb-1 text-[0.7rem] font-bold uppercase tracking-wide text-carvao/60">Tú dices</p>
    <button
      type="button"
      onclick={tocarSuaFala}
      aria-label={revelado ? `Reproducir: ${sua.es}` : 'Revelar la frase'}
    ...
    >
      ...
      {:else}
        ...
        <span class="mt-1 block text-[0.8rem] font-semibold text-oceano"
          >Esta ya la aprendiste — toca para revelarla</span
        >
      {/if}
    </button>
  {/if}
