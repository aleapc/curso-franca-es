<script lang="ts">
  ...
  async function ouvirFalar(ex: Extract<Exercise, { tipo: 'falar' }>, i: number) {
    ...
    try {
      const ouvido = await recognizeOnce('fr-FR');
      const ok = normalize(ouvido).includes(normalize(ex.alvo));
      falaResultado[i] = { ok, ouvido };
    } catch {
      falaResultado[i] = { ok: false, ouvido: '(no se pudo escuchar — revisa el micrófono)' };
    }
  }
    ...
    <div class="card p-4">
      ...
      {:else if ex.tipo === 'preencher'}
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          {#if ex.dica}<span class="text-xs text-carvao/50">pista: {ex.dica}</span>{/if}
        </div>
        {#if revelado[i]}
          <p class="mt-2 text-sm {preencherOk(ex, i) ? 'text-salvia' : 'text-terracota'}">
            {preencherOk(ex, i) ? '¡Correcto! ✔' : `Respuesta: ${ex.resposta}`}
          </p>
        {/if}
      ...
      {:else if ex.tipo === 'escuta'}
        <div class="mb-3 flex items-center gap-3">
          ...
          <span class="text-sm text-carvao/60">¿Qué has oído?</span>
        </div>
        <div class="grid gap-2">
          {#each ex.opcoes as op, o}
            <button
      ...
      {:else if ex.tipo === 'parear'}
        <p class="mb-3 font-medium">Toca un término y luego su pareja en español.</p>
        ...
        {#if pares[i] && Object.keys(pares[i]).length === ex.pares.length}
          <p class="mt-3 text-sm {ex.pares.every((_, e) => parOk(i, e)) ? 'text-salvia' : 'text-terracota'}">
            {ex.pares.every((_, e) => parOk(i, e))
              ? '¡Todo correcto! ✔'
              : 'Algunas parejas están cambiadas — toca de nuevo para rehacerlas.'}
          </p>
        {/if}
        ...
        {#if recognitionSupported()}
          ...
          {#if falaResultado[i]}
            <p class="mt-2 text-sm {falaResultado[i]?.ok ? 'text-salvia' : 'text-terracota'}">
              {falaResultado[i]?.ok ? '¡Muy bien! ✔' : 'Casi — inténtalo de nuevo.'}
              <span class="text-carvao/60"> (oí: “{falaResultado[i]?.ouvido}”)</span>
            </p>
          {/if}
        {:else}
          <p class="text-sm text-carvao/50">
            Este navegador no admite el reconocimiento de voz (prueba con Chrome en el móvil).
          </p>
        {/if}
      {/if}
    </div>
