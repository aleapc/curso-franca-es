<script lang="ts">
  ...
  const acao: Record<string, string> = {
    ouvir: 'Escucha',
    responde: 'Tu turno',
    shadow: 'Dilo con ellos'
  };
  function badge(s?: Step): string {
    if (!s) return '';
    if (s.tipo === 'intro' || s.tipo === 'recap') return vozNome[s.voz ?? ''] ?? 'Guía';
    return acao[s.tipo] ?? '';
  }
    ...
    if (typeof navigator !== 'undefined' && 'mediaSession' in navigator) {
      try {
        navigator.mediaSession.metadata = new MediaMetadata({
          ...
          artist: 'Dites-moi ! French',
          album: 'Dites-moi ! French'
        });
...
<div class="card mt-4 flex min-h-[220px] flex-col items-center justify-center gap-3 p-5 text-center">
  ...
  {#if fase === 'pausa' && step?.tipo === 'responde'}
    ...
    <p class="text-2xl font-extrabold text-terracota">🎤 ¡Habla ahora!</p>
    {#key index}
  ...
  {:else if fase === 'aguardando' && step?.tipo === 'responde'}
    ...
    {:else}
      <p class="text-sm text-carvao/50">¿Lo has dicho? Escucha al nativo o simplemente sigue.</p>
    {/if}
    {#if recUrl}
      <button class="btn bg-oceano text-white text-sm" onclick={ouvirVoce}>▶ escúchate</button>
    {/if}
  {:else if fase === 'pausa' && step?.tipo === 'shadow'}
    <p class="text-2xl font-extrabold text-terracota">🗣️ ¡Dilo con ellos!</p>
    <p class="text-sm text-carvao/50">intenta sin leer</p>
  {:else if ehPt}
  ...
  {#if erroAudio}
    <p role="status" class="text-xs font-medium text-terracota">
      ⚠️ Audio no disponible — comprueba tu conexión (o usa "guardar sin conexión" antes de salir).
    </p>
  {/if}
</div>
...
<div class="mt-4 flex items-center justify-center gap-3">
  ...
  {#if fase === 'aguardando'}
    <button class="btn-primary px-6 py-3" onclick={ouvirResposta}>▶ oír la respuesta</button>
    <button class="btn bg-salvia px-6 py-3 text-white" onclick={proximo}>⏭ siguiente</button>
  {:else}
    <button class="btn-primary px-8 py-3 text-lg" onclick={botaoCentral}>
      {#if fase === 'parado'}{index === 0 ? '▶ Empezar' : '▶ Continuar'}
      {:else if fase === 'fim'}↺ Otra vez
      {:else}⏸ Pausa{/if}
    </button>
  {/if}
...
<div class="mt-3 flex flex-wrap items-center justify-center gap-4 text-sm">
  <button class="-my-2 px-2 py-3 text-oceano" onclick={repetir}>⟲ repetir paso</button>
...
{#if micErro}
  <p role="status" class="mt-1 text-center text-xs text-terracota">
    ⚠️ Sin acceso al micrófono — actívalo en Ajustes del iPhone › Dites-moi ! › Micrófono.
  </p>
{/if}

<div class="mt-2 flex items-center justify-center gap-2 text-xs text-carvao/70">
  <span>Pausa para hablar:</span>
...
<div class="mt-3 flex justify-center">
  ...
  >
    {#if baixando}⬇ descargando… {baixaProg}/{clipKeys.length}
    {:else if baixado}✅ guardado sin conexión
    {:else}⬇ guardar esta parte sin conexión{/if}
  </button>
</div>

{#if modo === 'carro'}
  <p class="mt-3 text-center text-xs text-carvao/45">
    🚗 Modo coche: se reproduce solo de principio a fin. Usa los botones del volante o la pantalla
    de bloqueo para reproducir, pausar y saltar.
  </p>
{/if}

{#if fase === 'fim'}
  <p class="mt-5 text-center text-lg font-semibold text-salvia">✅ ¡Parte completada!</p>
{/if}
