...
<div class="card p-4">
  ...
  <div class="mt-3 flex flex-wrap items-center gap-2">
    <button type="button" class="btn-primary" onclick={escutar}>
      {tocando ? '⏸ Pausa' : emMeio || pos > 0 ? '▶ Continuar' : '▶ Escuchar la conversación'}
    </button>
    <button
      type="button"
    ...
    >
      {mostrarTranscricao ? 'Ocultar la transcripción' : 'Ver la transcripción'}
    </button>
  </div>

  {#if erroAudio}
    <p role="status" class="mt-2 text-xs font-medium text-terracota">
      ⚠️ Sin audio — comprueba tu conexión e inténtalo de nuevo.
    </p>
  {/if}
  ...
  {:else if tocando && idxFala >= 0}
    <p class="mt-3 text-center text-sm text-carvao/50">
      🔊 Reproduciendo… ({idxFala + 1}/{quiz.dialogo.length})
    </p>
  {/if}
</div>
  ...
  {#if !corrigido}
    ...
    >
      {tudoRespondido
        ...
        : `Respóndelas todas (${respostas.filter((r) => r !== null).length}/${quiz.perguntas.length})`}
    </button>
  {:else}
    <div class="card p-5 text-center">
      ...
      <p class="mt-1 text-carvao/70">
        {acertos === quiz.perguntas.length
          ? 'Perfecto. Lo has acertado todo. 🎉'
          : acertos >= quiz.perguntas.length - 2
            ? 'Muy bien. Casi lo tienes. 👏'
            : 'Buen comienzo — escucha otra vez e inténtalo de nuevo. 💪'}
      </p>
      <div class="mt-4 flex justify-center gap-2">
        <button type="button" class="btn bg-white ring-1 ring-black/10" onclick={refazer}>Reintentar</button>
        <button type="button" class="btn-primary" onclick={escutarDoInicio}>Escuchar otra vez</button>
      </div>
    </div>
  {/if}
</div>
