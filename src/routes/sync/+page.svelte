<script lang="ts">
  ...
  onMount(() => {
    ...
    if (!m) {
      estado = 'erro';
      detalhe = 'No hay ningún código en este enlace. Pide uno nuevo.';
      return;
    }
    codigo = m[1];
    ...
    if (!data) {
      estado = 'erro';
      detalhe = 'Código no válido — pídeles que generen uno nuevo.';
      return;
    }
    const standalone =
...
<div class="mt-10 text-center">
  ...
  {:else if estado === 'preview'}
    ...
    <p class="mt-2 text-carvao/70">
      Este código trae {previa?.ale ?? 0} + {previa?.dea ?? 0} partes completadas. Importar lo combina
      todo y no elimina nada.
    </p>
  ...
  {:else if estado === 'fora'}
    <h1 class="text-2xl font-extrabold">Casi está 📲</h1>
    <p class="mx-auto mt-2 max-w-sm text-carvao/70">
      Este enlace se abrió en tu <b>navegador</b>, pero tu progreso vive en la <b>app Dites-moi !</b> de
      tu pantalla de inicio. Copia el código y pégalo allí:
    </p>
    <ol class="mx-auto mt-3 max-w-sm space-y-1 text-left text-sm text-carvao/70">
      <li>1. Toca <b>Copiar código</b> abajo</li>
      <li>2. Abre la app <b>Dites-moi !</b> de tu pantalla de inicio (en un ordenador: la portada del curso)</li>
      <li>3. En <b>Sincronizar</b>, pégalo e impórtalo</li>
    </ol>
    ...
    >
  {:else}
    <h1 class="text-2xl font-extrabold text-terracota">Vaya… 🤔</h1>
    <p role="status" class="mt-2 text-carvao/70">{detalhe}</p>
  {/if}
