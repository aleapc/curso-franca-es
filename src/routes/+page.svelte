<script lang="ts">
  ...
  function importar() {
    ...
    msgOk = !!r;
    msg = r
      ? `Importado: +${r.ale} tuyos, +${r.dea} suyos. 🎉`
      : 'Código no válido — comprueba que lo copiaste entero (empieza por ES1.).';
  }
...
<section class="mt-2">
  ...
  <p class="mt-1 text-carvao/70">
    Francés de supervivencia y cultura para Francia: escucha, dilo en voz alta y que no te la cuelen.
  </p>
</section>
...
>
  ...
  <span class="min-w-0 flex-1">
    ...
    <span class="block text-xs text-carvao/60"
      >Instala <b>Google Translate</b> y descarga el <b>francés</b> para usarlo sin conexión: la cámara lee
      cartas, etiquetas y formularios al momento.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">↗</span>
</a>
<p class="mt-1 px-1 text-[11px] text-carvao/45">
  💡 Configúralo ANTES de volar. Te salva con la letra pequeña, pero no te pedirá la cerveza ni te
  dirá que la misma bebida cuesta tres precios distintos.
</p>
...
>
  ...
  <span class="min-w-0 flex-1">
    <span class="block font-bold leading-tight text-carvao">Bolsillo del viajero</span>
    <span class="block text-xs text-carvao/60"
      >Moneda, tipo de enchufe y el número de emergencias: funciona con el móvil en modo avión.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">→</span>
</a>
  ...
  <section class="mt-7">
          ...
          <div class="space-y-1.5">
            ...
            {#if quizDoEpisodio[ep.id] && ep.partes.some((p) => p.pronta)}
              ...
              >
                ...
                <span class="flex-1 text-sm font-medium">Quiz del episodio</span>
    ...
    {#if examDoNivel[nivel.nivel] && nivel.episodios.some((e) => e.partes.some((p) => p.pronta))}
      ...
      >
        ...
        <span class="flex-1">
          <span class="block text-sm font-bold">Prueba de {nivel.nome}</span>
          <span class="block text-xs opacity-90">Audio más largo + prueba de comprensión</span>
        </span>
...
<section class="mt-8">
  <div class="card p-4">
    <h2 class="mb-1 font-bold">👯 Sincroniza con tu compañero de viaje</h2>
    ...
    <div class="flex flex-wrap gap-2">
      <button type="button" class="btn-primary" onclick={gerar}>Consigue mi código</button>
      {#if codigo}
