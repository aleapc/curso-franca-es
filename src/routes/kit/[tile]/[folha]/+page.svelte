...
<svelte:head>
  <title>{data.folha.rotulo} · Dites-moi !</title>
</svelte:head>
  ...
  <div class="min-h-0 flex-1 space-y-2.5 overflow-y-auto overscroll-contain pb-1">
    ...
    {:else}
      ...
      <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/10">
        <p class="text-[1.05rem] font-bold leading-snug">Aquí todavía no hay nada.</p>
        <p class="mt-1 text-[0.9rem] leading-snug text-carvao/75">
          Esto está en el mapa, pero sus frases aún no han llegado al kit. Puede que el resto de
          {data.tile.rotulo.toLowerCase()} ya tenga lo que necesitas.
        </p>
        <a
        ...
        >
          <span class="text-oceano">◀</span> Volver a {data.tile.rotulo}
        </a>
      </div>
    {/if}
  </div>
