  ...
  {#snippet rodape()}
    <div class="flex items-end gap-2">
      ...
      {#if chaves.length}
        ...
        >
          ...
          {:else if estado === 'pronto'}✓ Viaje
          {:else if estado === 'parcial'}↻ Viaje
          {:else if estado === 'sem-suporte'}— Viaje
          {:else}⬇ Viaje{/if}
        </button>
      {/if}
    </div>
    {#if estado === 'parcial'}
      <p role="status" class="mt-1 text-center text-[0.72rem] font-semibold text-terracota">
        Algunos audios no se descargaron — toca ↻ Viaje otra vez mientras tengas cobertura.
      </p>
    {:else if estado === 'sem-suporte'}
      <p role="status" class="mt-1 text-center text-[0.72rem] font-semibold text-terracota">
        Este navegador no puede guardar el audio. Añade antes la app a tu pantalla de inicio.
      </p>
    {/if}
  {/snippet}
</KitTela>
